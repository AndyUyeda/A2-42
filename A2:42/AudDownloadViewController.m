...
@interface AudDownloadViewController () <MFMessageComposeViewControllerDelegate>
...
@synthesize theTitle, websiteURL, currentSermon, startTime,endTime, pauseButton,minuteLabel,minuteText,secondLabel,secondText,hourLabel,hourText, undoButton,goButton,teacher,favoriteStar,type,startTimeLabel,cropButton,shareButton,imman,copButton;
...
-(void)initPlayer
{
    ...
    if([type isEqualToString:@"mp3"]){
        if([theTitle containsString:@"m4a"]){
            ...
            NSLog(@"%@",[NSString stringWithFormat:@"%@%@%@",@"Documents/",theTitle,@".mp3"]);
        }
    }
    else{
        [cropButton setHidden:TRUE];
        [copButton setHidden:TRUE];
        [favoriteStar setHidden:TRUE];
...
- (void)viewDidLoad
{
    ...
    if([theTitle containsString:@"m4a"]){
        currentSermon.text = [theTitle substringFromIndex:3];
    }
    else{
...
- (IBAction)sought:(id)sender {
    if(seekAction == 0){
        ...
        [startTimeLabel setHidden:FALSE];
        [startTimeLabel setTitle:ss forState:UIControlStateNormal];
        [goButton setHidden:FALSE];
        [secondText setHidden:FALSE];
        [secondLabel setHidden:FALSE];
        [minuteText setHidden:FALSE];
    ...
    if([type isEqualToString:@"mp3"]){
        ...
        if([[[alert textFieldAtIndex:0] text] length] > 0){
            ...
            if([imman isEqualToString:@"Immanuel"]){
                NSString *iii = [NSString stringWithFormat:@"immanuel%@%@",teacher,[[alert textFieldAtIndex:0]text]];
            ...
            [currentSermon setText:[[alert textFieldAtIndex:0]text]];
            currentSermon.numberOfLines = 0;
            currentSermon.textAlignment = NSTextAlignmentCenter;
            [currentSermon sizeToFit];
            currentSermon.center = CGPointMake([self view].center.x, 134);
...
- (BOOL)exportAsset:(AVAsset *)avAsset toFilePath:(NSString *)filePath {
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^{
        ...
        if (AVAssetExportSessionStatusCompleted == exportSession.status) {
            ...
            if([imman isEqualToString:@"Immanuel"]){
                NSString *iii = [NSString stringWithFormat:@"immanuel%@%@",teacher,croppedTitle];
                [prefs setBool:true forKey:iii];
            }
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([segue.identifier isEqualToString:(@"notes")]){
        ...
        if([theTitle containsString:@"m4a"]){
            NSString* ttt = [theTitle substringFromIndex:3];
            NSLog(@"%@",ttt);
            note.theTitle = ttt;
        }
        else{
